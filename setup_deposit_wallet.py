...
Account-wide (one shared wallet across every configured city — see
core/execution.py:build_client()), not tied to any single city's config.

Required in .env (or as env vars):
  POLYMARKET_PRIVATE_KEY   your EOA private key (the same one the bot trades from)
...
After --approve succeeds, update the bot's own .env:
...
# multi-bracket weather markets (what this bot trades) are typically
...
def cmd_approve(relayer):
    ...
    print()
    print("Deposit wallet setup complete. Update the bot's .env:")
