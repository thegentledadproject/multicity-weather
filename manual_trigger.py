...
(e.g. confirming the SELL/NO-token-BUY mechanics actually post successfully)
without waiting for the scheduler's own cron ticks.

WHAT IT DOES:
  1. Fetches the Gamma event for the given city (config.cities.CITIES) and
     date (same slug logic as core/discovery.py) and extracts the given
     bracket's YES/NO token ids.
  2. Builds an authenticated CLOB client (core/execution.py's build_client()) —
     the same account-wide client used for every city.
  3. Runs the exact same ExecutionEngine.execute() path the scheduler calls
     from core/city_runner.py's job_order_execution() — same VWAP/ghost-book/
     drift checks, same fill parsing, same DB position recording — just with
     a fixed $ size and direction instead of a computed EdgeSignal/SizingResult.
  4. Requires typed confirmation before signing/posting (skip with --yes,
     e.g. for a scripted/cron re-run — use with care).

Usage:
    python manual_trigger.py --icao WSSS --bracket 31C --date 2026-07-18 --side YES --size 1.00
    python manual_trigger.py                      # defaults: WSSS, 31C, 2026-07-18, YES, $1.00
...
def _parse_args():
    ...
    p.add_argument("--icao", default="WSSS", help="City ICAO code from config.cities.CITIES (default: WSSS)")
...
def main():
    ...
    try:
        ...
        print(f"ERROR: '{args.icao}' is not a configured city. See config.cities.CITIES.")
        sys.exit(1)

    if bracket not in city_config.bracket_labels:
        print(f"ERROR: '{bracket}' is not a tradeable bracket label for {args.icao}. "
              f"Valid: {city_config.bracket_labels}")
        sys.exit(1)

    ledger = Ledger()
    discovery = MarketDiscovery(ledger, city_config)

    print(f"Fetching token matrix for {args.icao} on {args.date} ...")
    matrix = discovery._fetch_from_gamma(args.date)
    if not matrix:
        print(f"ERROR: no event/markets found for {args.icao} on {args.date} "
              f"(Gamma slug/browse fetch returned nothing).")
        sys.exit(1)
    if bracket not in matrix:
        print(f"ERROR: bracket {bracket} not found in {args.icao}'s {args.date} event. "
              f"Found brackets: {list(matrix.keys())}")
        sys.exit(1)
    ...
    ledger.upsert_token_matrix(bracket, yes_id, no_id, args.date, icao=args.icao)

    print(f"\nAbout to place a REAL order: BUY {args.side} {bracket} for {args.icao} "
          f"(date={args.date}) for ${args.size:.2f}")
    ...
    print(f"\nExecuting BUY {args.side} {bracket} ${sizing.size_usd:.2f} for {args.icao} ...")
    filled = engine.execute(signal, sizing, market_date=args.date)

    if filled:
        print(f"\n✓ FILLED — position recorded for {args.icao} {bracket} [{args.side}].")
    else:
