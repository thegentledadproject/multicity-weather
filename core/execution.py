...
def _extract_vwap_ask(orderbook: Any, max_spend_usd: float) -> Optional[float]:
    ...
    # guarantee ask ordering. Confirmed against a real fill in production:
    # an unsorted book produced a computed VWAP of 0.999 for a BUY that
    # actually filled at ~0.07 (the real best ask) — the worst-price limit
    # derived from that bad VWAP happened to be generous enough that the
    # fill still executed at the true, better price, but the bug corrupted
    # every downstream EV/edge number logged for that trade and could, for
    # a SELL, produce an unsafe (too-lenient) worst-price limit instead.
    def _ask_price(a):
        return float(a.price) if hasattr(a, "price") else float(a.get("price", 0.0))

    raw_asks = sorted(raw_asks, key=_ask_price)
...
def _parse_fill_status(response: Any, label: str) -> bool:
    ...
    Confirmed against a REAL matched order: py_clob_client_v2's post_order()
    response for a filled FOK can have NO "size_matched" key at all — e.g.
        {'status': 'matched', 'success': True,
         'takingAmount': '14.285713', 'makingAmount': '0.999999',
         'transactionsHashes': ['0x...'], 'orderID': '0x...', 'errorMsg': ''}
    Reading only response.get("size_matched", 0.0) defaults to 0.0 for this
    real shape, so `matched > 0` is always False — meaning a genuinely
    successful fill gets logged and treated as a rejection, and
    record_position() never runs for a real trade. Fixed to also recognize
    takingAmount/makingAmount and the "success" flag, falling back to the
    old size_matched key if a different response shape ever supplies it.
    """
    if response is None:
...
class ExecutionEngine:
    ...
    def execute(
        ...
        # on-chain state on its own — it can go stale mid-process (not just
        # across restarts), causing "balance: 0" rejections on a long-running
        # process even after a prior sync succeeded hours earlier.
        #
        # A sync call alone is NOT enough: production logs (single-city
        # predecessor of this codebase) showed update_balance_allowance()
        # return 200 OK, immediately followed (~250ms later) by post_order()
        # still rejecting with "balance: 0" — while a separate
        # get_balance_allowance() call made ~10s later (via the dashboard)
        # read the real, nonzero, on-chain-correct balance.
